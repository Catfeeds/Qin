<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:60:"D:\wwwroot\wchhui1\wwwroot/app/wap/view/index/guestbook.html";i:1494491126;s:56:"D:\wwwroot\wchhui1\wwwroot/app/wap/view/Public/base.html";i:1495342088;}*/ ?>
﻿<!DOCTYPE html>
<html>
...
<style>
	...
	<div class="mui-content">
		<div class="mui-content-padded">
			<div class="mui-inline">给TA留言</div>
		</div>
		<div class="row mui-input-row">
			<textarea id='text' class="mui-input-clear question" placeholder="留个脚印吧..." style="height: 200px;"></textarea>
		</div>
		<div class="mui-content-padded">
			<button id='btn' data-loading-text="发表中" class="mui-btn mui-btn-block mui-btn-primary">发表</button>
		</div>
	</div>
...
<script type="text/javascript" charset="utf-8">
	document.getElementById("btn").addEventListener('tap', function(){
		...
		if(text){
			...
			//ajax交互，进行登录
			mui.post('<?php echo url('index/guestbook'); ?>',{
					text:text,id:'<?php echo $userid; ?>'
				},function(data){
					mui("#login").button('reset');
					if(data.status == 1) {
						mui.toast(data.message);
						location.href="<?php echo url('Index/user',['user_id'=>$userid]); ?>";
					}else{
						mui.toast(data.message);
					}
