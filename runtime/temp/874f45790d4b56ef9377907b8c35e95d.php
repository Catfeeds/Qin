<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:56:"D:\wwwroot\qnysj1\wwwroot/app/wap/view/user/article.html";i:1501228676;s:55:"D:\wwwroot\qnysj1\wwwroot/app/wap/view/Public/base.html";i:1498016252;}*/ ?>
﻿<!DOCTYPE html>
<html>
	...
	<div class="mui-content">
		<div style="padding: 10px 10px;">
			<div id="segmentedControl" class="mui-segmented-control">
				<a class="mui-control-item" href="<?php echo url('user/record'); ?>">来访记录</a>
				<a class="mui-control-item mui-active" href="<?php echo url('user/article'); ?>">动态</a>
				<a class="mui-control-item" href="<?php echo url('user/guestbook'); ?>">留言</a>
				<a class="mui-control-item" href="<?php echo url('user/photo'); ?>">相册</a>
			</div>
		</div>
		<div id="ass" style="padding: 10px 10px;">
		<a style="width:100%;" href="<?php echo url('user/article_add'); ?>" class="mui-btn mui-btn-primary">发布动态</a>
		</div>
		...
		<ul id="ajax_guestbook_return" class="mui-table-view">
			...
			<li class="mui-table-view-cell">
				...
				<a  href="/wap/index/user_art_info/user_id/<?php echo input('session.m_user_auth.id'); ?>.html?art_id=<?php echo $vo['id']; ?>" style="color:inherit;" >
					<em>
					<?php if($vo['json_img']!='/'): ?>
					<div style="float:left;"><img src="<?php echo $vo['json_img']; ?>" /></div>
					<?php endif; ?>
					<div style="float:left;width: 62%;max-height: 105px;"><?php echo mb_substr($vo['esc'],0,40,'utf-8'); ?>...</div>
					</em>
					<div style="clear:both;"></div>
					<p><em><?php echo date('Y-m-d h:s',$vo['add_time']); ?></em>
					<?php if($vo['back_3']==1): ?>
					<em style="float: right;color:red;font-size: 16px;margin-left: 5px;">[精华]</em><?php endif; ?>
					<em style="float: right;"><?php echo $vo['com_number']; ?>评论/<?php echo $vo['p_number']; ?>转发</em></p>
				</a>
				</div>
			</li>
...
<script type="text/javascript" charset="utf-8">
		...
		$('#ajax_guestbook_return').on('tap', '.mui-btn', function(event) {
			...
			mui.confirm('确认删除该条动态？', '通知信息', btnArray, function(e) {
				if (e.index == 0) {
					...
					mui.ajax('<?php echo url('User/article_del'); ?>',{
						data:{id:id},
	...
	mui("#getmore").on('tap','a',function(){
	 /*** ajax 提交表单 查询列表结果*/  
			...
			page += 1;
			mui.ajax('<?php echo url('user/guestbook'); ?>',{
				data:{
					is_ajax:1,
					page:page
				},
