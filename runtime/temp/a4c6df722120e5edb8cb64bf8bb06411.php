<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:63:"D:\wwwroot\qnysj1\wwwroot/app/wap/view/index/user_art_info.html";i:1501469744;s:55:"D:\wwwroot\qnysj1\wwwroot/app/wap/view/Public/base.html";i:1498016252;}*/ ?>
﻿<!DOCTYPE html>
<html>
...
<section>
	...
	<?php if($art_obj['back_3']==1): ?>
	<a style="color:red;font-size: 22px;margin: 6px 0px 0px 5px;position: absolute;">[精华]</a>
	<?php endif; ?>
	<a href="/wap/index/user_art_zf/art_id/<?php echo $art_obj['id']; ?>.html?user_id=<?php echo $obj['id']; ?>" class="mui-btn mui-btn-primary mui-btn-outlined" style="float: right;margin: 0 8px 0 8px;">转发</a>
	<a href="/wap/index/user_art_rep/art_id/<?php echo $art_obj['id']; ?>.html?user_id=<?php echo $obj['id']; ?>" class="mui-btn mui-btn-primary mui-btn-outlined" style="float: right;">举报</a>
	<p>
	<ul class="uMyList <?php if($obj['id'] != \think\Session::get('m_user_auth.id')): ?>Cndy<?php endif; ?>">
        <li class="manage" id="content">
        	<?php echo $art_obj['content']; ?>
			<div style="color: #999;margin-left: 5px;margin-right: 5px;height: 50px;">
				<em style="float: right;"><?php echo date('Y-m-d',$art_obj['add_time']); ?></em>
				<br/>
				<em style="float: right;"><?php echo $art_obj['com_number']; ?>评论/<?php echo $art_obj['p_number']; ?>转发</em>
			</div>
        </li>
    </ul>
...
<a style="width:100%;" href="/wap/index/com_add/art_id/<?php echo $art_obj['id']; ?>.html?user_id=<?php echo $obj['id']; ?>" class="mui-btn mui-btn-primary">评论</a>
</div>
<!--footer-->
...
mui("#getmore").on('tap','a',function(){
	 /*** ajax 提交表单 查询列表结果*/  
	page += 1;
	mui.ajax('<?php echo url('index/user_art_Info'); ?>',{
		data:{
			user_id:id,
			is_ajax:1,
			page:page
