<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:64:"D:\amp\UPUPW_K2.1_64\htdocs\QN/app/admin\view\module\p_list.html";i:1492499346;s:62:"D:\amp\UPUPW_K2.1_64\htdocs\QN/app/admin\view\public\base.html";i:1493099283;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
  <ul class="layui-tab-title">
    <li onclick="javascript:window.location.href='<?php echo url('module/art_list',['id'=>$mid]); ?>'">社团动态</li>
...
<table class="layui-table" lay-even>
  ...
  <tbody>
    ...
    <tr>
	  ...
	  <td><?php echo !empty($vo['type']) && $vo['type']==1?'回复':'评论'; ?></td>
	  <td><?php if(($vo['status'] == 1)): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
      <td >
	  <dl class="layui-btn-group">
		  <a href="<?php echo url('user/p_xx',['id'=>$vo['id']]); ?>" class="layui-btn">详细</a>
		  <a href="<?php echo url('user/z_list',['id'=>$vo['id'],'comid'=>$aid]); ?>" class="layui-btn">点赞记录</a>
  ...
  layui.use(['form'], function(){
	...
	form.on('switch(yyy)', function(data){
	  ...
	  if(data.elem.checked)
		...
		$.ajax({
			url:"<?php echo url('module/p_update'); ?>",
			type:"post",
			dataType:"json",
	...
	function del(id)
	{
			layer.confirm('删除之后不可恢复，是否继续执行？', {
			  ...
			  btn1:function(index, layero){
					$.ajax({
					url:"<?php echo url('module/p_delete'); ?>",
					type:"post",
					dataType:"json",
					data:{aid:id},
					success:function(dd){
