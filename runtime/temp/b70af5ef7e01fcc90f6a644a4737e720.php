<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:66:"D:\amp\UPUPW_K2.1_64\htdocs\QN/app/admin\view\module\art_list.html";i:1492499200;s:62:"D:\amp\UPUPW_K2.1_64\htdocs\QN/app/admin\view\public\base.html";i:1493099283;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <tr>
	 ...
	 <td><?php if(($vo['status'] == 1)): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
	  ...
	  <dl class="layui-btn-group">
		  <a href="<?php echo url('module/art_xx',['id'=>$vo['id']]); ?>" class="layui-btn">详细</a>
		  <!--<a href="<?php echo url('module/z_list',['id'=>$vo['id'],'mid'=>$aid]); ?>" class="layui-btn">浏览记录</a>-->
		  <a href="<?php echo url('module/p_list',['id'=>$vo['id'],'mid'=>$aid]); ?>" class="layui-btn">评论记录</a>
	...
	form.on('switch(yyy)', function(data){
	  ...
	  if(data.elem.checked)
		...
		else
		{
			...
			$.ajax({
				url:"<?php echo url('module/art_update'); ?>",
				type:"post",
				dataType:"json",
	...
	function del(id)
	{
			layer.confirm('删除之后不可恢复，是否继续执行？', {
			  ...
			  btn1:function(index, layero){
					$.ajax({
					url:"<?php echo url('module/art_delete'); ?>",
					type:"post",
					dataType:"json",
					data:{aid:id},
					success:function(dd){
