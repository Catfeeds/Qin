...
class Player extends Controller
{
    ...
    public function index()
    {
		...
		if(request()->isPost())
		{
			...
			//是否已报名?
			$obj=db('player')->where('user_id='.$session_user_id." and type=".$data['type_id']." and maxid=".$data['maxid']." and minid=".$data['minid'])->find();

			if($obj)
			{
				$this->error('您已报名：'.db('type')->where('id',$data['type_id'])->value('name').' '.db('type')->where('id',$data['maxid'])->value('name').' '.db('type')->where('id',$data['minid'])->value('name').' '.'大赛,无法重复提交');
			}
